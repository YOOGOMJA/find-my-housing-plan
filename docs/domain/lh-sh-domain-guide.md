...
- 이 문서는 현재 구현 상태를 설명하는 문서가 아니라, 구현 전에 고정해야 할 도메인 기준을 정리한 가이드다.
- 각 항목은 아래 세 종류가 섞여 있다.
  - `사실`: 법령, 기관 소개, 공고문, API 문서로 직접 확인 가능한 내용
  - `운영 정의`: 자동화 설계를 위해 팀이 채택한 해석 기준
  - `권장 사항`: 스키마/수집 방식/체크리스트처럼 구현에 반영할 제안
- `운영상 추론`, `권장`, `추가 확인 필요`로 표시된 항목은 구현 전 또는 구현 중 다시 검증한다.
...
- 어떤 개념을 `사업`으로 보고, 어떤 단위를 `공고 이벤트`로 관리할지
- `공고일 현재` 같은 기준일 개념을 어떤 필드로 저장할지
- 사용자 입력이 필요한 자격정보와 외부 데이터로 수집 가능한 정보를 어디서 나눌지
- 누락 정보가 있을 때 `판정불가`를 언제 반환할지
...
아래 정의는 실무 자동화 관점의 운영 정의이며, 충돌 시 `법령/고시 원문 > 공고문 원문 > 포털 요약` 우선순위를 적용한다.
...
먼저 `8-1 ~ 8-3`은 공고/사업 자체에서 수집해야 하는 항목이고, `8-4`는 사용자에게 직접 받아야 하는 항목이다.
...
아래 항목은 아직 이 문서만으로 확정되지 않은 주제다. 구현 전에 별도 검증 또는 보조 문서화가 필요하다.
...
- 법적/운영 최종 판단은 `법령/고시 원문 > 공고문 원문 > 포털 요약` 우선순위를 따른다.
- 소득/자산 기준 수치는 연도별로 변동될 수 있으므로, 기준표는 별도 버전 관리가 필요하다.
- 구현이 진행되더라도, 실제 코드 구조나 파일 경로 자체는 이 문서의 책임 범위가 아니다.
