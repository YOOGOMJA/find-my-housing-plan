...
이 문서를 읽고 나면 최소한 아래 세 가지가 바로 결정되어야 한다.

- 어떤 개념을 시스템의 핵심 도메인 용어로 고정할지
- 어떤 정보를 외부 데이터로 수집하고 어떤 정보를 사용자 입력으로 받을지
- 누락 정보와 예외 케이스를 어디까지 자동 판정하고 어디서 멈출지
...
구현 문맥에서 이 섹션을 볼 때는 아래 순서로 해석한다.

1. `8-1 ~ 8-2`: 수집기/파서가 채워야 할 데이터
2. `8-3`: 판정 엔진이 사용해야 할 룰 데이터
3. `8-4`: 사용자 프로필이나 증빙 흐름이 채워야 할 데이터
...
- 용어 사전: 도메인 enum/코드 후보 정리 문서
- 판정 입력 스키마: 사용자/공고/주택 정보 필드 정의 문서
- 판정 사유 코드 사전: `판정불가`, 탈락, 통과 사유 코드 목록
- 파서 추출 항목 목록: 원문에서 반드시 뽑아야 할 필드 목록
